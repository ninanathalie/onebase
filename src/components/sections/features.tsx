...
const Feature = ({ title, description, icon, index }: { title: string; description: string; icon: React.ReactNode; index: number }) => {
  return (
    <div className={cn(
        ...
        (index === 0 || index === 3) && "lg:border-l lg:border-b-0 dark:border-slate-800",
        index < 3 && "lg:border-b dark:border-slate-800",
        index < 4 ? "md:border-b" : "md:border-b-0",
        index < 5 && "border-b"
        )}>
      {index < 3 && <div className="opacity-0 group-hover/feature:opacity-100 transition duration-200 absolute inset-0 h-full w-full bg-gradient-to-t from-primary-50 dark:from-primary-500/10 to-transparent pointer-events-none" />}
      {index >= 3 && <div className="opacity-0 group-hover/feature:opacity-100 transition duration-200 absolute inset-0 h-full w-full bg-gradient-to-b from-primary-50 dark:from-primary-500/10 to-transparent pointer-events-none" />}

      <div className="mb-4 relative z-10 px-10 text-slate-600 dark:text-slate-300">{icon}</div>
      ...
      <div className="text-lg font-semibold mb-2 relative z-10 px-10">
        ...
        <span className="group-hover/feature:translate-x-2 transition duration-200 inline-block text-slate-800 dark:text-slate-300">{title}</span>
      </div>

      <p className="text-slate-600 dark:text-slate-400 md:max-w-xs relative z-10 px-10">{description}</p>
    </div>
  );
};
